...
async function sendAndroidPush(userId: string, title: string, detail: string, link?: string) {
  ...
  try {
    const response = await fetch("https://exp.host/--/api/v2/push/send", {
      ...
      body: JSON.stringify(devices.map((device) => ({ to: device.token, title, body: detail, sound: "paper_plane_landing.wav", channelId: "breakroom-alerts-v2", priority: "high", data: { url: link ?? "/notifications" } }))),
    });
